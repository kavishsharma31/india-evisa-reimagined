...
test('all eight A01 routes use citizen-facing requirements and fee copy', async ({ page }) => {
  ...
  for (const purpose of PURPOSES) {
    ...
    await expect(page.getByText('Varies by nationality and visa category', { exact: true })).toBeVisible()
    await expect(page.getByText('Visa fees vary by nationality and visa category.', { exact: true })).toHaveCount(0)
