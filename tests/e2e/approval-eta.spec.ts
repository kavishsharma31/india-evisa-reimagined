...
test('A09 issued outcome stays full-width, readable, overflow-free, and axe-clean across target viewports', async ({ page }) => {
  ...
  for (const width of [360, 390, 430, 768, 1280]) {
    ...
    const approvedPanel = page.getByRole('region', { name: 'Demo application approved' })
    const etaPanel = page.getByRole('region', { name: 'Synthetic ETA issued' })
    const watermark = etaPanel.getByText('SYNTHETIC — NOT VALID', { exact: true })
    const warning = page.getByText(
      'SYNTHETIC — NOT VALID. This is not a visa or travel document.',
    )
    const statusWidth = (await statusPage.boundingBox())?.width ?? 0
    const approvedWidth = (await approvedPanel.boundingBox())?.width ?? 0
    const etaWidth = (await etaPanel.boundingBox())?.width ?? 0
    expect(approvedWidth).toBeGreaterThan(statusWidth - 2)
    expect(etaWidth).toBeGreaterThan(statusWidth - 2)
